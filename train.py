...
parser.add_argument('--epoch', type=int, default=3, help='epoch number')  # 训练轮数，默认为0~2
...
parser.add_argument('--model_kind', type=str, default='RNN_GRU', help='kind of model, i.e. CNN or RNN_LSTM or mlp or Transformer')  # 模型类型，可以选择‘CNN’、‘RNN_LSTM’、'CRNN_LSTM’、'Transformer'、‘MLP’等、
...
def main():
    ...
    if opt.model_kind == 'RNN_LSTM':  # rnn即LSTM网络
        ...
        model = RNN_LSTM(n_features=features.shape[1], seq_length=features.shape[0], n_categories=category_counts)
    ...
    elif opt.model_kind == 'CRNN_LSTM':  # CRNN系列网络使用和RNN相同的输入特征
        dataset = load_or_generate_data_RNN(filename='RNN_features/RNN_features_train.pickle', root1=opt.data_root_1, root2=opt.data_root_2, type='train')
        dataset1 = load_or_generate_data_RNN(filename='RNN_features/RNN_features_test.pickle', root1=opt.data_root_1, root2=opt.data_root_2, type='test')
        features, _ = dataset[6]  # 获取其中一个样本的特征向量，将其维度传入model，以便于构建合适的输入维度
        model = CRNN_LSTM(n_features=features.shape[1], seq_length=features.shape[0], n_categories=category_counts)
    elif opt.model_kind == 'CRNN_GRU':  # CRNN系列网络使用和RNN相同的输入特征
        dataset = load_or_generate_data_RNN(filename='RNN_features/RNN_features_train.pickle', root1=opt.data_root_1, root2=opt.data_root_2, type='train')
        dataset1 = load_or_generate_data_RNN(filename='RNN_features/RNN_features_test.pickle', root1=opt.data_root_1, root2=opt.data_root_2, type='test')
        features, _ = dataset[6]  # 获取其中一个样本的特征向量，将其维度传入model，以便于构建合适的输入维度
        model = CRNN_GRU(n_features=features.shape[1], seq_length=features.shape[0], n_categories=category_counts)
    elif opt.model_kind == 'RNN_GRU':  # RNN系列网络使用和RNN相同的输入特征
        dataset = load_or_generate_data_RNN(filename='RNN_features/RNN_features_train.pickle', root1=opt.data_root_1, root2=opt.data_root_2, type='train')
        dataset1 = load_or_generate_data_RNN(filename='RNN_features/RNN_features_test.pickle', root1=opt.data_root_1, root2=opt.data_root_2, type='test')
        features, _ = dataset[6]  # 获取其中一个样本的特征向量，将其维度传入model，以便于构建合适的输入维度
        model = RNN_GRU(n_features=features.shape[1], n_categories=category_counts)
