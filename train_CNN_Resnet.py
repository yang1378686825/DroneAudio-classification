...
from model_CNN_Resnet import SegDataset_MelSpecCNN, ResNetLike1D, MelSpecCNN
from model_GRU import CRNN_GRU, RNN_GRU
from model_Transformer import Transformer
from models import RNN_LSTM, MLP
...
parser.add_argument('--epoch', type=int, default=5, help='epoch number')  # 训练轮数，默认为0~2
parser.add_argument('--lr', type=float, default=2e-4, help='learning rate')  # 学习率，默认为1e-3
parser.add_argument('--batch_size', type=int, default=16, help='training batch size')  # 批次大小，默认为16
...
parser.add_argument('--model_kind', type=str, default='ResNetLike1D', help='kind of model, i.e. CNN or RNN_LSTM or mlp or Transformer')  # 模型类型，可以选择‘CNN’、‘RNN_LSTM’、'CRNN_LSTM’、'Transformer'、‘MLP’等、
...
def main():
    ...
    dataset = SegDataset_MelSpecCNN(root1=opt.data_root_1, root2=opt.data_root_2, type='train')
    dataset1 = SegDataset_MelSpecCNN(root1=opt.data_root_1, root2=opt.data_root_2, type='test')
    features, _ = dataset[6]  # 获取其中一个样本的特征向量，将其维度传入model，以便于构建合适的输入维度

    if opt.model_kind == 'ResNetLike1D':
        model = ResNetLike1D(seq_length=features.shape[0], n_categories=category_counts)
    elif opt.model_kind == 'MelSpecCNN':
        model = MelSpecCNN(seq_length=features.shape[0], n_categories=category_counts)
    ...
    for epo in range(opt.epoch):
        ...
        for index, (features, label) in enumerate(train_iter):
            ...
            if np.mod(index, 5) == 0 or index == len(train_iter) - 1: # 添加检查是否为最后一个迭代
                print('epoch {}, {}/{}batches, iter train loss is {}'.format(epo, index, len(train_iter), iter_loss))
